...
public class CardioChallengeRestController {
	...
	public ResponseEntity<List<CardioChallenge>> listAllCardiosByLevel(@PathVariable("level") String level) {
		System.out.println("Fetching cardio with level " + level);
		List<CardioChallenge> cardios = cardioService.findAllCardiosByLevel(level);
		if (cardios.isEmpty()) {
